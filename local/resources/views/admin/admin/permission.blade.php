                        ...
                        <div class="col-md-6">
                            <div class="box box-default box-solid" id="permission-user-{{ $value['id'] }}">
                                <div class="box-header with-border">
                                    <h3 class="box-title"><input type="checkbox" id="chkAll_{{ $value['id'] }}" name="navigation_id[]" value="{{ $value['id'] }}" class="checkAll" {{ $key == 0 ? "disabled" : "" }} {{ $key == 0 ? "checked" : (in_array($value["id"], $adminPermissions) ? "checked" : "") }} />  {{ App::getLocale() == "en" ? $value['en_name'] : $value['name'] }}</h3>
                                ...
                                <div class="box-body">
                                    @if(isset($value["children"]) && count($value["children"]))
                                        @foreach($value["children"] as $row)
                                            <label class="col-sm-4 small_label"><input type="checkbox" id="chkAll_{{ $row['id'] }}" name="navigation_id[]" value="{{ $row['id'] }}"  {{ in_array($row['id'], $adminPermissions) ? 'checked' : '' }}/>  {{ App::getLocale() == "en" ? $row['en_name'] : $row['name'] }}</label> 
                                            @if(isset($row["children"]) && count($row["children"]))
                                                @foreach($row["children"] as $rowVal)
                                                    <label class="col-sm-4 small_label"><input type="checkbox" id="chkAll_{{ $rowVal['id'] }}" name="navigation_id[]" value="{{ $rowVal['id'] }}"  {{ in_array($rowVal['id'], $adminPermissions) ? 'checked' : '' }}/>  {{ App::getLocale() == "en" ? $rowVal['en_name'] : $rowVal['name'] }}</label>              
                                                @endforeach
                                            @endif
                                        @endforeach
                                    @endif
...
<script type="text/javascript">
    jQuery(function($) {
      $(document).on('click','#updatePermissionBtn',function(e){
        e.preventDefault()
        $.ajax({
            url: "{{ route('admin.admin.permission', ['id' => $admin->id ]) }}",
