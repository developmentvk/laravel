<div class="modal-content" id="edit_admin_modal_box">
   <div class="modal-header">
      ...
      <h4 class="modal-title">{{ cpTrans('update') }} <span class="department_name_box"></span></h4>
   </div>
         ...
         <div class="col-md-12">
            ...
            <form class="form-horizontal" id="update-admin-modal-form" enctype="multipart/form-data">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <input type="text" class="form-control" name="mobile" placeholder1="{{ cpTrans('mobile') }}" value="{{ $admin->mobile }}">
                </div>
              </div>
              <div class="form-group">
                  ...
                  <div class="col-sm-8">
                      <div class="form-inline">
                          @foreach(cpTrans('action_status') as $key => $status)
                              <div class="radio">
                                  <label>
                                      <input type="radio" name="account_status" {{ $key == $admin->status ? "checked" : "" }} value="{{ $key }}"> {{ $status }}
                                  </label>
                              </div>
                          @endforeach
                      </div>
              ...
              @endif
              <input type="hidden" name="account_access_id" value="{{ $admin->department_id }}">
            </form>
         </div>
      </div>
   </div>
...
<script type="text/javascript">
    ...
    var account_access_id = accountAccessData.account_access_id != undefined ? accountAccessData.account_access_id : {{ $admin->department_id }};
    var department_name = accountAccessData.department_name != undefined ? accountAccessData.department_name : '';
    $("#edit_admin_modal_box [name=account_access_id]").val(account_access_id);
    $("#edit_admin_modal_box .department_name_box").html(`${department_name} {{ cpTrans('account') }}`);
    $("#edit_admin_modal_box").on('click','#adminModalUpdateBtn',function(e){
       e.preventDefault();
       var department_id = $('[name=account_access_id]').val();
       $.ajax({
            url: "{{ route('admin.admin.update', ['id' => $admin->id]) }}",
            ...
            success: function (data) {
                ...
                $(`#admin-table-${department_id}`).DataTable().ajax.reload(null, false);
